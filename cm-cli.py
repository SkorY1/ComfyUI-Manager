...
class Ctx:
    ...
    def set_channel_mode(self, channel, mode):
        ...
        unified_manager.reload(cache_mode=self.mode == 'cache')
...
def install_node(node_spec_str, is_all=False, cnt_msg=''):
    if '://' in node_spec_str:
        # install via urls
        res = core.gitclone_install([node_spec_str])
        if not res:
        ...
        res = unified_manager.install_by_id(node_name, version_spec, channel_ctx.channel, channel_ctx.mode, instant_execution=True)
...
def show_list(kind, simple=False):
    ...
    for k, v in unified_manager.active_nodes.items():
        if flag:
            ...
            processed[k] = "[    ENABLED    ] ", cnr['name'], k, cnr['publisher']['id'], v[0]
        else:
            processed[k] = None
    ...
    for k, v in unified_manager.cnr_inactive_nodes.items():
        ...
        if flag:
            cnr = unified_manager.cnr_map[k]
            processed[k] = "[    DISABLED   ] ", cnr['name'], k, cnr['publisher']['id'], ", ".join(list(v.keys()))
        else:
            processed[k] = None
    ...
    for k, v in unified_manager.nightly_inactive_nodes.items():
        ...
        if flag:
            cnr = unified_manager.cnr_map[k]
            processed[k] = "[    DISABLED   ] ", cnr['name'], k, cnr['publisher']['id'], 'nightly'
        else:
            processed[k] = None
    ...
    for k, v in unified_manager.cnr_map.items():
        ...
        if flag:
            ...
            processed[k] = "[ NOT INSTALLED ] ", cnr['name'], k, cnr['publisher']['id'], ver_spec
        else:
            processed[k] = None
...
def restore_snapshot(
    ...
    try:
        ...
        with open(snapshot_path, 'r', encoding="UTF-8") as snapshot_file:
            ...
            if cnr_info is not None:
                ...
                for k, v in cnr_info.items():
                    ps = unified_manager.install_by_id(k, version_spec=v, instant_execution=True, return_postinstall=True)
                    if ps is not None:
                        postinstalls.append(ps)
        ...
        # reload
        unified_manager.reload(cache_mode=channel_ctx.mode == 'cache')
